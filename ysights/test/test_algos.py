...
class AlgosTestCase(unittest.TestCase):
    ...
    def test_paradox_per_degree_class_custom_bins(self):
        ...
        results = visibility_paradox_per_degree_class(
            handler, network, N=10, bins=custom_bins
        )
        self.assertIsInstance(results, dict)
        self.assertIn("bin_edges", results)
    ...
    def test_paradox_temporal(self):
        ...
        results = visibility_paradox_temporal(
        ...
        print(
            f"Temporal paradox computed for {len(results['time_points'])} time windows"
        )
        print(f"Temporal granularity: {results['temporal_granularity']}")
        if len(results["time_points"]) > 0:
            print(
                f"First time point: Day {results['time_points'][0][0]}, Hour {results['time_points'][0][1]}"
            )
    ...
    def test_paradox_temporal_custom_granularity(self):
        ...
        results = visibility_paradox_temporal(
            handler, network, temporal_granularity=(0, 12), N=5
        )
        self.assertIsInstance(results, dict)
